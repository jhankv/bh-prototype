...
- Mock data lives beside the view and is shaped like production data.
- **Views may not use the shell's Tailwind classes.** `src/shell.css` scans only
  `src/`, on purpose. A prototype styles itself through its sandbox.
...
by hand. `frame.css` is ours.

*(Sandboxes arrive in Phase 2. Until then `sandbox` is `"none"`.)*
